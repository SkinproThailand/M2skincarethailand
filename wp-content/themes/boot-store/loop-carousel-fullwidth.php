...
<?php if ( ! have_posts() ) : ?>
      <article id="post-0" class="post no-results not-found">
          ...
          <div class="entry-content">
              <p><?php _e( 'Apologies, but no results were found for the requested archive. Perhaps searching will help find a related post.', 'bre-bootstrap-ecommerce' ); ?>
            </p>
              <?php get_search_form(); ?>
          </div><!-- .entry-content -->
      </article><!-- #post-0 -->
...
<?php $currency = tcp_the_currency( false ); 
...
if ( isset( $instance['title_tag'] ) && $instance['title_tag'] != '' ) {
	$title_tag = '<' . $instance['title_tag'] . ' class="slide-title">';
...
if ( $see_sorting_panel ) {
	tcp_the_sort_panel();
} ?>

<?php /* Start the Loop.*/ ?>

<div id="fullCarousel" class="carousel fullCarousel slide">
	<div class="carousel-inner">

	<?php $class = 'active'; ?>
	<?php while ( have_posts() ) : the_post(); ?>
		<div class="item <?php echo $class; ?> bs-slide-<?php the_ID(); ?>">
			<?php $class = ''; ?>
			<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
				...
				<div class="row-fluid wrapper-group">
					...
					<div class="span8">
						...
						<?php if ( $see_meta_utilities ) : ?>
							...
							<p>
								...
								<span class="comments-link btn btn-small"><?php comments_popup_link( '<span class="leave-reply">' . __( 'Leave a review', 'bre-bootstrap-ecommerce' ) . '</span>', __( '<b>1</b> Review', 'bre-bootstrap-ecommerce' ), __( '<b>%</b> Reviews', 'bre-bootstrap-ecommerce' ) ); ?></span>								<?php endif; // End if comments_open() ?>
	...
	<?php endwhile; // End the loop ?>
	 ...
	 <!-- Carousel nav -->
	<a class="carousel-control left" href="#fullCarousel" data-slide="prev">&lsaquo;</a>
	<a class="carousel-control right" href="#fullCarousel" data-slide="next">&rsaquo;</a>
</div><!-- #myCarousel -->

		<script type="text/javascript">
		jQuery( function() {
			jQuery( '#fullCarousel' ).carousel( {
				interval: 8000
			} );
		} );
		</script>
